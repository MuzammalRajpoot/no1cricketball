		...
		if (isset($this->request->get['token']) && isset($this->session->data['token']) && ($this->request->get['token'] == $this->session->data['token'])) {
			...
			*/
			...
			// Design
				...
				$tool_id = $this->model_extension_module->getModulesByCode('so_tools');
				$magentech = array();
				...
				if ($this->user->hasPermission('access', 'extension/module/so_newletter_custom_popup')) {		
				 ...
				 if ($this->user->hasPermission('access', 'extension/module/so_tools')) {		
					$magentech[] = array(
						'name'	   => 'So Tools',
						'href'     => $this->url->link('extension/module/so_tools', 'token=' . $this->session->data['token'].'&module_id='.$tool_id[0]['module_id'], true),
						'children' => array()
					);
				} 
				...
				if ($this->user->hasPermission('access', 'extension/module/so_categories')) {		
					$magentech[] = array(
						'name'	   => 'So Categories',
						'href'     => $this->url->link('extension/module/so_categories', 'token=' . $this->session->data['token'], true),
						'children' => array()
					);
				}
